// c
/*
 Simple registration server (Linux).
 Usage: ./4_server <tcp_port>
 Protocol (over TCP):
...
static int parse_registration(const char *line, char *out_ip, size_t iplen, int *out_port, int sockfd) {
    ...
    if (colon) {
        ...
        *out_port = atoi(colon + 1);
        ...
        /* only port provided; get peer IP from sockfd */
